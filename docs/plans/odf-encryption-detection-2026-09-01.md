...
Landed as `4b22a29` (S1–S6), `248ac29` (A10 cache poison, S1 real golden), `b8d115b` (A10 control fixtures). Arc [#1](https://github.com/Slurp9187/odf-decrypt-rs/issues/1), slices [#2](https://github.com/Slurp9187/odf-decrypt-rs/issues/2)–[#7](https://github.com/Slurp9187/odf-decrypt-rs/issues/7). Still open and linked, not attached: [#8](https://github.com/Slurp9187/odf-decrypt-rs/issues/8) (OQ2) and [#9](https://github.com/Slurp9187/odf-decrypt-rs/issues/9) (OQ3), both corpus-gated and both carried by the decrypt arc.
...
3. **PGP + SHA512-1K.** `ZipPackage::setPropertyValue` defaults PGP checksum to `SHA512_1K` (`ZipPackage.cxx` 1920–1923) but `ManifestExport` only writes SHA1-1K / SHA256-1K. Unknown checksum-type ⇒ no digest-alg ⇒ non-GCM PGP fails the accept predicate. Likely the save path overrides to GCM before export; not fully traced. Tracked as [#9](https://github.com/Slurp9187/odf-decrypt-rs/issues/9), free-standing (linked, not attached) because it gates the decrypt arc as well. No detection verdict depends on it: `classify` already matches LO on read for both the non-GCM (incomplete either way) and GCM (checksum ignored either way) cases.
