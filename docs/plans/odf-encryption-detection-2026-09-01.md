...
Landed as `4b22a29` (S1–S6), `248ac29` (A10 cache poison, S1 real golden), `b8d115b` (A10 control fixtures). Arc [#1](https://github.com/Slurp9187/odf-crypto/issues/1), slices [#2](https://github.com/Slurp9187/odf-crypto/issues/2)–[#7](https://github.com/Slurp9187/odf-crypto/issues/7). [#8](https://github.com/Slurp9187/odf-crypto/issues/8) (OQ2) and [#9](https://github.com/Slurp9187/odf-crypto/issues/9) (OQ3) closed 2026-09-02 from the LO pin.
...
⚠️ Nested `foo/content.xml` would also latch. Specified-by-LO (short name); OQ2 closed — no save path emits a nested-only latch, and a third-party file that did would still be latched by LO, so matching the short name stays correct. A10 is a second route: a folder miss poisons `m_aRecent` so a nested-path *row* can latch **root** `content.xml` (tested). An embedded object can add a second latch-capable row (`Object N/content.xml`) alongside the root; first-wins reports whichever the manifest lists first. No golden has an embedded object, so `common` on a multi-latch package is untested against a real file.
...
- **`classify` is normal load only.** `m_bForceRecovery` is false. Repair (`ZipPackage.cxx` 461–465, 469, 512, 540; the `\`→`/` rewrite at 636–640) is a different product; do not add a recovery mode to this crate. Every Group B finding assumes `m_bForceRecovery == false`.
- **`m_bHasNonEncryptedEntries` stays off `Classification`.** Incomplete tuples never enter `encrypted_entries`; a later decrypt crate treats everything else as copy-through.

  > **Amended 2026-09-21 (rc.5 §4): the sentence above is circular, and checking upstream shows its conclusion is wrong.** It argues from *this project's own* `decrypt` copy-through to conclude nobody needs the flag, which cannot fail — whatever `decrypt` does becomes the reason. Re-argued from LibreOffice instead, which is what this file's own authority rule requires.
  >
  > The flag is **not** dead upstream. It is set at `ZipPackage.cxx:446` and read in exactly one place, `SfxObjectShell::CheckEncryption_Impl` (`sfx2/source/doc/objmisc.cxx:1028-1063`): on ODF >= 1.2, when `HasEncryptedEntries && HasNonEncryptedEntries`, LibreOffice raises `ERRCODE_SFX_INCOMPLETE_ENCRYPTION` — *"this is an encrypted document with nonencrypted streams inside, show the warning"* — and then calls `disallowMacroExecution()`. That is a security decision, not bookkeeping.
  >
  > `Classification` carries `package_encrypted` (`HasEncryptedEntries`) and nothing for the other half, so a consumer **cannot tell whether LibreOffice would raise that warning**: the predicate needs both flags and only one is exposed. That is a known limitation of the shape this plan chose, of the same kind the `m_bMediaTypeFallbackUsed` bullet below already records. Filed as [#60](https://github.com/Slurp9187/odf-crypto/issues/60) to be closed by adding the field or by documenting the unreachable behaviour on `package_encrypted`.
- **`m_bMediaTypeFallbackUsed` stays off `Classification`.** Known limitation, not the same fact: it records whether the root media-type came from the manifest or was sniffed off the `mimetype` stream (`ZipPackage.cxx` 503–507). `Classification.media_type` has no provenance, so a consumer cannot tell a declared type from a fallback one.
- **Remaining `readCEN` checks stay noted, not promised.** Overlapping entries (`ZipFile.cxx` 1436–1481), STORED size mismatch (`:1427-1430`), data-descriptor holes (`:1521+`), `Count != Total`, name length. Unaudited; same family as the checks above (invalid names and duplicate CEN were major and implemented because answering for archives LO refuses lets a crafted file pick its verdict). Risk is unquantified, not known-low.
...
2. **Nested `content.xml` latch.** Closed 2026-09-02 from `07047a02f94d` ([#8](https://github.com/Slurp9187/odf-crypto/issues/8)). The latch stays the short name (`getName()` is the last path component, `ZipPackageEntry.cxx` 53–55). No corpus evidence could change that: a third-party file whose only complete latch row is `Object N/content.xml` would still latch in LO, so matching LO is still correct. The corpus gate was only ever deciding whether to add a special case; there is no input that would justify one. Calligra, Collabora, and older StarOffice are unsurveyed, and it does not matter.

   LO/AOO save never emits the nested-*only* case. Wholesome hides nested `content.xml` inside the `encrypted-package` blob, so `classify` never sees it; latch is `encrypted-package`. Per-entry save writes a root `content.xml` and encrypts it under the common password (`UseCommonStoragePasswordEncryption` in xstor: `owriteablestream.cxx` / `xstorage.cxx`). An embedded object can add `Object N/content.xml` *alongside* the root; if the manifest lists the nested row first, first-wins reports it as `common`. That is specified-by-LO, not OQ2. No golden has an embedded object, so `common` on a multi-latch package is untested against a real file.

   A10 is a second route to a nested-*path* row latching **root** `content.xml` (`m_aRecent` folder-miss poison). That one is tested. The nested-only latch remains constructible and already covered by those fixtures; it is not a producer shape.
3. **PGP + SHA512-1K.** Closed 2026-09-02 from `07047a02f94d` ([#9](https://github.com/Slurp9187/odf-crypto/issues/9) close-when 1). `ENCRYPTION_GPG_PROPERTIES` does default `m_oChecksumDigestID` to `SHA512_1K` and the cipher to AES-CBC (`ZipPackage.cxx` 1917–1923), with a comment that `SfxObjectShell::SetupStorage` should override. That override is real and always later on the sfx2 save path (`objstor.cxx` 315–405 after `GetStorage`/`GetOutputStorage` has already applied GPG via `SetCommonStorageEncryptionData`): ODF ≥ 1.2 wholesome → AES-GCM and checksum cleared; ODF ≥ 1.2 per-entry → AES-CBC + SHA256-1K; older → Blowfish + SHA1-1K. `ENCRYPTION_ALGORITHMS_PROPERTY` takes only SHA1-1K or SHA256-1K: an empty `Any` clears the digest (`ZipPackage.cxx` 1887–1891 — that reset is how wholesome GCM ends up with no checksum at all), and any other value throws `IllegalArgumentException` (`:1892-1896`). So SHA512-1K cannot be put back through the public setter. `ManifestExport.cxx` 377–382 throws on any other digest id — it cannot emit a SHA512-1K `checksum-type`. `ManifestImport::doEncryptionData` has no SHA512 alias. `ZipFile::StaticGetDigestContextForChecksum` only constructs SHA1-1K / SHA256-1K contexts; NSS can compute SHA512-1K (`nssinitializer.cxx`) but package write never asks. A UNO caller that skipped `SetupStorage` would still fail at export, not write a file. **`Checksum` does not gain a variant. Plan §4 is complete.** Decrypt of LO-written PGP is GCM with no checksum (wholesome current) or CBC + SHA256-1K (ODF 1.2 per-entry). Hand-crafted unknown `checksum-type` still classifies as today: incomplete unless GCM.
