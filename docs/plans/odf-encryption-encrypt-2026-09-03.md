...
2. **`getrandom` vs. an already-vendored CSPRNG.** `aes-gcm`'s `aead` trait crate may already re-export `OsRng` under some feature combination, which could avoid adding a new named dependency. Decide during S1 by checking what's already reachable in the resolved dependency graph before adding `getrandom`; whichever is chosen, name it explicitly in `Cargo.toml` under the `encrypt` feature only (mirrors the decrypt-arc audit's `flate2` finding: an unnamed or accidentally-default dependency is a defect, not a convenience).
