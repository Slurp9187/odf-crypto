Status: **Shipped (2026-09-03)** — modern-only password encryption; per-entry write (Blowfish/AES-CBC) is a later arc · Landed in [#24](https://github.com/Slurp9187/odf-crypto/pull/24) (`7bd6a89` implementation, `9686ff5`/`1198be6` review fixes, `37e7bc5` the secure-gate merge), closing arc [#18](https://github.com/Slurp9187/odf-crypto/issues/18) and slices [#19](https://github.com/Slurp9187/odf-crypto/issues/19)–[#23](https://github.com/Slurp9187/odf-crypto/issues/23) · Authored 2026-09-03 against `07047a02f94d` · Review round 2026-09-03: every LO citation re-grepped against source (several line ranges in round 1 had drifted, two by more than 20 lines), emit table added, S5 gating fixed, mimetype question settled
...
1. **Per-entry write (Blowfish CFB / AES-CBC).** Real LO behavior, fully characterized in §1's table and already cited in §Authority so the arc that implements it does not have to re-derive the primitives. Not started.

   > **Amended 2026-09-21 (rc.5 §4).** This used to close on *"a concrete reason to target an older ODF version on write (there is none today)"*. That is demand, asserted — unfalsifiable from inside the repo, and it puts the burden on a user who has no way to register the need. It also reads as a decision against per-entry write when the truth is that nobody has built it. The crate ships goldens of all three read profiles and round-trips one, so the asymmetry is visible in the test corpus while the plan called it settled. Replaced with an effort/gap statement and filed as [#59](https://github.com/Slurp9187/odf-crypto/issues/59) so the gap is an open item rather than a closed question. The decision may well stand; the reasoning was not a reasoning.

   **Effort/gap, stated instead.** Every primitive exists — `cbc`, `blowfish`, `cfb-mode` and `pbkdf2` are already in the `crypto-ops` graph because `decrypt` uses them, and §1's table has `SetupStorage`'s defaults. What does not exist is the emit side: fresh per-member salt and IV (never one pair reused across members, per **Do not copy** above), one `encryption-data` per encrypted member instead of one per package, and `manifest:size` on each.
